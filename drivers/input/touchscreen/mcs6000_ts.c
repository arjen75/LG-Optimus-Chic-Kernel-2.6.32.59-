...
extern struct vc_data* get_vc_data();
...
void display_update_msg(int nMsg);
void lge_melfas_vdd(UINT8 enable);
#endif
...
/* _S [kyuhyung.lee] 2010.02.23 : to support touch event from UTS*/
...
static int mcs6000_ts_suspend(struct device *device)
{
	struct mcs6000_ts_device *dev = i2c_get_clientdata(client);
...
static int mcs6000_ts_resume(struct device *device)
{
	struct mcs6000_ts_device *dev = i2c_get_clientdata(client);
...
#ifndef CONFIG_HAS_EARLYSUSPEND
static struct dev_pm_ops mcs6000_ts_pm_ops = {
...
// _S dangwoo.choi - Melfas Firmware Update
#ifdef LGE_MELFAS_UPDATE_ENABLE
void display_update_msg(int nMsg)
{
	struct vc_data* vc;
	...
	switch(nMsg) {
	case 0 : // intro
		fbcon_putcs_byLGE (vc,"O l d   T o u c h S c r e e n   F / W   i s   d e t e c t e d ! ",32,1,0);
		fbcon_putcs_byLGE (vc,"F / W   U p d a t e   w i l l   t a k e   a b o u t   4 0 s e c . ",33,2,0);
	break;
	case 1 : // success
		fbcon_putcs_byLGE (vc,"[ R e s u l t ]   S U C C E S S ! ! ! ",19,4,0);
	break;
	case 2 : // failed
		fbcon_putcs_byLGE (vc,"[ R e s u l t ]   F A I L E D . . . ",18,4,0);
	break;
	case 3 : // i2c fail
		fbcon_putcs_byLGE (vc,"[ E R R ]   T O U C H   I 2 C   F A I L ! ",21,1,0);
	break;
	}
...
void lge_melfas_enable_isr(UINT8 enable)
{
	if(enable) {
		DMSG("melfas ENABLE ISR\n");
		enable_irq(mcs6000_ts_dev.num_irq);
	}
	else {
		DMSG("melfas DISABLE ISR\n");
		disable_irq(mcs6000_ts_dev.num_irq);
	}
}

void lge_melfas_watchdog_reset(UINT8 enable)
{
...
{
	if(enable)
		mcs6000_ts_dev.power(1);
	else
		mcs6000_ts_dev.power(0);	
}
...
void lge_melfas_i2c_set(UINT8 dest, UINT8 set)
{
	int gpio, nErr;

	if(dest == TKEY_I2C_SCL) {
		DMSG("melfas set SCL ");
		gpio = mcs6000_ts_dev.scl_gpio;
	}
	else {
		DMSG("melfas set SDA ");
		gpio = mcs6000_ts_dev.sda_gpio;
	}
	...
	case TKEY_SET_OUTPUT :
		DMSG("to OUTPUT\n");
		nErr = gpio_configure(gpio, GPIOF_DRIVE_OUTPUT);
		if(nErr) {
			DMSG("GPIO_CONFIGURE FAILED (%d)\n", nErr);
		}
		break;
	case TKEY_SET_INPUT :
		DMSG("to INPUT\n");
		nErr = gpio_configure(gpio, GPIOF_INPUT);
		if(nErr) {
			DMSG("GPIO_CONFIGURE FAILED (%d)\n", nErr);
		}
		break;
...
BOOLEAN lge_melfas_i2c_read(UINT8 slave_addr, UINT8 *pData, UINT8 cLength)
{
	int nAddr = mcs6000_ts_dev.client->addr, nErr;

   	mcs6000_ts_dev.client->addr = slave_addr;
	nErr = i2c_smbus_read_byte(mcs6000_ts_dev.client);
	if(nErr < 0) {
		DMSG("lge_melfas_i2c_read ERROR addr=%x (%d)\n", mcs6000_ts_dev.client->addr, nErr);
